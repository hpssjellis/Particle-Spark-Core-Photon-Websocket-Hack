...
char server[] = "ml-websocket-01--jerteach.repl.co"; // Node server needs to allow users to have read access to your url
...
//////////////////////////////////// global variables /////////////////////////////// 
 ...
 int myDriveSpeed = 0;
 int myMinDriveSpeed = 25;   // minimum drive speed unless stopped
 int myNormalDriveSpeed = 30;
 int myMaxDriveSpeed = 60;
...
void loop() {
 ...
 // All commands from webpage
    ...
    // 9 commands to work with the webpage
        ...
        if (myIncoming == 'p'){  myDriveSpeed -= 5; myTurnAmount += -300;             myTurnSpeed = myTurnNormalSpeed; }   // this must set on website
        if (myIncoming == 'q'){  myDriveSpeed -= 5;  myTurnAmount = myTurnStraight; myTurnSpeed = myTurnNormalSpeed; }   
        if (myIncoming == 'r'){  myDriveSpeed -= 5; myTurnAmount += 300;             myTurnSpeed = myTurnNormalSpeed; }   
        
        if (myIncoming == 's'){  myDriveSpeed = myNormalDriveSpeed; myTurnAmount += -300;            myTurnSpeed = myTurnNormalSpeed; }   
        if (myIncoming == 't'){  myDriveSpeed = myNormalDriveSpeed; myTurnAmount = myTurnStraight; myTurnSpeed = myTurnNormalSpeed; }  
        if (myIncoming == 'u'){  myDriveSpeed = myNormalDriveSpeed; myTurnAmount +=  300;            myTurnSpeed = myTurnNormalSpeed; }   

        if (myIncoming == 'v'){  myDriveSpeed += 5; myTurnAmount += -300;            myTurnSpeed = myTurnNormalSpeed; }   
        if (myIncoming == 'w'){  myDriveSpeed += 5; myTurnAmount = myTurnStraight; myTurnSpeed = myTurnNormalSpeed; }   
        if (myIncoming == 'x'){  myDriveSpeed += 5; myTurnAmount += 300;            myTurnSpeed = myTurnNormalSpeed; }   
