...
void loop() {
   ...
   if (mySafetyCount >= 4000){  // no connection for a while stop motors
        mySafetyCount = 0;
        myLeftMotorSpeed = 0;
        myRightMotorSpeed = 0;
        mySetMotors();
  ...
  if (client.connected()) {
    if (client.available()) {
        ...
        if (myIncoming == 'J'){ myLeftMotorSpeed=100; myRightMotorSpeed=100; mySetMotors(); }  // OK
        if (myIncoming == 'K'){ myLeftMotorSpeed=150; myRightMotorSpeed=150; mySetMotors(); }  // medium

        if (myIncoming == 'L'){ myLeftMotorSpeed=200; myRightMotorSpeed=200; mySetMotors(); }  // fast
        if (myIncoming == 'M'){ myLeftMotorSpeed=255; myRightMotorSpeed=255; mySetMotors(); }  // fastest

        if (myIncoming == 'Q'){ myBrake=0; myLeftMotorSpeed+=10; myRightMotorSpeed-=10; mySetMotors(); }  // left
        if (myIncoming == 'R'){ myBrake=0; myLeftMotorSpeed+=50; myRightMotorSpeed-=50; mySetMotors(); }  // big left
        if (myIncoming == 'S'){ myBrake=0; myLeftMotorSpeed-=10; myRightMotorSpeed+=10; mySetMotors(); }  // right
        if (myIncoming == 'T'){ myBrake=0; myLeftMotorSpeed-=50; myRightMotorSpeed+=50; mySetMotors(); }  // big right

        if (myIncoming == 'U'){ myBrake=1; mySetMotors(); }  // coast
        if (myIncoming == 'V'){ myBrake=2; mySetMotors(); }  // power break
        if (myIncoming == 'W'){ myBrake=0; myLeftMotorDirection=1; myRightMotorDirection=1; mySetMotors(); }  // set forward
        if (myIncoming == 'X'){ myBrake=0; myLeftMotorDirection=2; myRightMotorDirection=2; mySetMotors(); }  // set backward
